...
```js
let target = {}
const isEmptyObj = (target) => JSON.stringify(target) === "{}"
isEmptyObj(target)
```
...
- 2.复杂数据类型
  ...
  - instanceof 的特性：只要在当前实例的原型链上，我们用其检测都会有
...
console.log((128).toString(2)) // "10000000"，转换为 2 进制
console.log((128).toString(8)) // "200"，转换为 8 进制
console.log((128).toString(10)) // "128"，转换为 10 进制
```
